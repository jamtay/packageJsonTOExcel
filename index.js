...
console.warn('USAGE: node index.js  currentExcelFileName outputFileName jsonfilepath/package.json')
...
const getCSVStringFromDependency = (type) => {
  ...
  Object.entries(json[type]).forEach(dependency => {
    const currentValue = result.Sheet1.find(row => row.name === dependency[0])
    ...
    if (currentValue) {
      returnString += `"${dependency[0]}",${dependency[1]},${devOrProduction(type)},${currentValue.description}\n`
    } else {
      returnString += `"${dependency[0]}",${dependency[1]},${devOrProduction(type)},\n`
    }
  })
...
const returnString = getCSVStringFromDependency('dependencies') + getCSVStringFromDependency('devDependencies')
